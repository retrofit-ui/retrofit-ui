...
describe('ColumnSchema', () => {
  it('parses a valid column', () => {
    const result = ColumnSchema.safeParse({ key: 'name', label: 'Name', type: 'string' })
    expect(result.success).toBe(true)
  })

  it('defaults sortable and filterable to false', () => {
    const col = ColumnSchema.parse({ key: 'name', label: 'Name', type: 'string' })
    expect(col.sortable).toBe(false)
    expect(col.filterable).toBe(false)
  })

  it('rejects unknown column type', () => {
    const result = ColumnSchema.safeParse({ key: 'x', label: 'X', type: 'json' })
